...
fn success_restart_game() {
    ...
    for i in 1..MAX_NUMBER_OF_TURNS {
        if i == 3 {
            ...
            if debug_me {
                println!(
                    "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  res = {:?}",
                    res
                );
            }
            break;
        ...
        let nanos = (SystemTime::now()
            ...
            + 1;
        if debug_me {
            println!("Random number of pebbles that user will remove: {nanos}");
        }
        ...
        if debug_me {
            println!("{:?} user chose {:?} pebbles: ", i, user_choice);
        }
    }
    let nanos = (SystemTime::now()
        .duration_since(UNIX_EPOCH)
